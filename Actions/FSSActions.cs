    ...
    public static class FSSActions
    {
            ...
            string[] reportPath)
        {
            ...
            else if (IsParent(parentArg, "AM"))
            {
                ...
                {
                    if (!AdminChangeSite.TryGetValue("10.1", out var entryFunc))
                    {
                    ...
                    for (int i = 0; i < CommonEntryHelpers.Sites.Length; i++)
                    {
                        ...
                        await entryFunc(page, CommonEntryHelpers.Sites[i]);

                        // await ClickNavLinks.ClickNavLinksAsync(page, new[]
                        // {
                        //     FSSNavLinksActionsDict.Parents["RP"],
                        //     FSSNavLinksActionsDict.Children["RP"]["S"],
                        //     "7.2.1 Salesman Daily Sales Report (RPT07)"
                        // });

                        // Logger.Log("✅ Navigated to Salesman Daily Sales Report");

                        // await SalesmanDailySalesReportEntryHelper.ExecuteSalesmanDailySalesReportEntryAsync(page, CommonEntryHelpers.Sites[i]);
